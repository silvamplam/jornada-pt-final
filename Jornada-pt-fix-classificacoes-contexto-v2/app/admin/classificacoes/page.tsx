...
const standingsAdminStyles = `
  ...
  .standing-create-form {
    grid-template-columns: minmax(260px, 1.4fr) minmax(260px, 1.4fr) minmax(220px, 1fr) 120px;
  }

  .standing-meta-form {
    grid-template-columns: minmax(240px, 1.3fr) minmax(260px, 1.4fr) minmax(220px, 1fr) 120px;
...
function errorMessage(error?: string) {
  ...
  if (error === "missing-fields") {
    return "Epoca e jornada devem pertencer ao mesmo contexto competitivo.";
  }

  if (error === "invalid-context") {
    return "A epoca e a jornada escolhidas nao pertencem ao mesmo contexto. Escolhe uma combinacao coerente.";
  }
...
export default async function AdminStandingsPage({ searchParams }: StandingsPageProps) {
  ...
  const defaultMatchdayId = overview.matchdays.find((matchday) => matchday.season_id === defaultSeasonId)?.id ?? "";
    ...
    <main className="standings-admin-shell">
      ...
      <section className="standings-admin-create">
        <header>
          ...
          <small>Cria uma fotografia da tabela para uma epoca e jornada. A competicao vem da epoca escolhida.</small>
        </header>
        <form action="/api/admin/standings" className="standing-create-form" method="post">
          ...
          {selectField("new-matchday", "Jornada", "matchday_id", defaultMatchdayId, matchdayOptions(overview), !canWrite)}
