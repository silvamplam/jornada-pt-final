...
-- Aplicar: liga o recálculo automático do ranking demo
-- à função de escrita public.portal_upsert_result_entry(...).
--
-- Este SQL preserva exatamente o retorno existente da função:
-- TABLE(
...
--
-- Estratégia:
-- - manter a escrita de resultados na função existente;
-- - depois de cada upsert de resultado demo, chamar:
...
is 'Controlled Portal das Escolas result upsert for existing event participants. Requires active can_edit permission scoped to the event; validated status also requires can_validate. Demo competition results automatically trigger demo ranking recalculation.';
