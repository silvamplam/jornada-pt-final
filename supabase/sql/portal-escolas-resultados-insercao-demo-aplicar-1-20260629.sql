...
-- Aplicar: cria funcao controlada para inserir/atualizar resultados por evento/participante.
-- Nao altera schema de tabelas. Nao recalcula rankings. Nao mexe no backoffice.
...
begin
  ...
  insert into public.portal_result_entries (
    ...
    p_points,
    v_outcome,
    case when v_outcome = 'win' then true when v_outcome is null then null else false end,
    now(),
...
is 'Controlled Portal das Escolas result upsert for existing event participants. Requires active can_edit permission scoped to the event; validated status also requires can_validate.';
