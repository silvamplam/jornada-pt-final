...
--
-- Objetivo:
-- - remover a chamada automática a public.portal_recalculate_demo_competition_ranking()
--   de public.portal_upsert_result_entry(...);
-- - preservar a assinatura/retorno atual da função;
-- - manter o grant execute para authenticated;
-- - não apagar a função de recálculo criada na fase anterior.
--
-- Este rollback não altera tabelas, RLS, policies ou grants fora da função de escrita.
...
begin
  ...
  insert into public.portal_result_entries (
    ...
    jsonb_build_object(
      ...
      'rollback_phase', 'PORTAL-ESCOLAS-RANKING-AUTO-RECALCULO-DEMO-1',
...
is 'Controlled Portal das Escolas result upsert for existing event participants. Requires active can_edit permission scoped to the event; validated status also requires can_validate. Rollback version without automatic demo ranking recalculation.';
