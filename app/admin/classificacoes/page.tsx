...
const standingsAdminStyles = `
  ...
  .standing-create-form {
    grid-template-columns: repeat(4, minmax(180px, 1fr)) 120px;
  }
...
function errorMessage(error?: string) {
  ...
  if (error === "missing-fields") {
    return "Competicao e epoca sao obrigatorias.";
  }
...
function competitionOptions(standings: Awaited<ReturnType<typeof getAdminStandingsEditor>>) {
  return standings.competitions.map((competition) => (
...
function seasonOptions(standings: Awaited<ReturnType<typeof getAdminStandingsEditor>>) {
  return standings.seasons.map((season) => {
...
function matchdayOptions(standings: Awaited<ReturnType<typeof getAdminStandingsEditor>>) {
  return (
    <>
    ...
    <main className="standings-admin-shell">
      ...
      <section className="standings-admin-create">
        <header>
          ...
          <small>Cria uma fotografia da tabela para uma competicao, epoca e jornada.</small>
        </header>
        <form action="/api/admin/standings" className="standing-create-form" method="post">
          ...
          {selectField("new-competition", "Competicao", "competition_id", defaultCompetitionId, competitionOptions(overview), !canWrite, true)}
          {selectField("new-season", "Epoca", "season_id", defaultSeasonId, seasonOptions(overview), !canWrite, true)}
          {selectField("new-matchday", "Jornada", "matchday_id", "", matchdayOptions(overview), !canWrite)}
        ...
        {overview.standings.map((standing) => (
          <article className="standing-block" key={standing.id}>
            ...
            <form action={`/api/admin/standings/${standing.id}`} className="standing-meta-form" method="post">
              ...
              {selectField(`competition-${standing.id}`, "Competicao", "competition_id", standing.competition_id, competitionOptions(overview), !canWrite, true)}
              {selectField(`season-${standing.id}`, "Epoca", "season_id", standing.season_id, seasonOptions(overview), !canWrite, true)}
              {selectField(`matchday-${standing.id}`, "Jornada", "matchday_id", standing.matchday_id, matchdayOptions(overview), !canWrite)}
